...
data = load_dataset(dataset_path=data_path, batch_size=batch_size, spatial_sample_num=spatial_sample_num, eps=eps,
                    T_in=T_in, T_pred=T_pred, resample_resolution=[8, 8], equispace=equispace, sub=sub, device=device,
                    meshgrid_inv_num=spatial_sample_num*2)
...
trainer = Trainer(data=data,
                  model=model,
                  log_dir='experiments/',
                  experiment_name='VIT3d_neq_s{}_ti{}_to{}'.format(sub, T_in, T_pred),
                  optimizer=optimizer,
